     ...
     look AS ( SELECT meta.dictionary_standard_attribute_values.value AS key
      ...
      uri AS ( SELECT meta.dictionary_standard_attribute_values.value AS uri,
               ...
               WHERE  meta.standard_attribute.attribute = 'dict_uri'
                 AND  $3::timestamptz(3) <@ meta.dictionary_standard_attribute_values.validity )
SELECT                replace(uri.uri, '{{LOOKUP}}', asset.orchestration_environment_unique_attribute_values.value) AS resource
FROM                  asset.orchestration_environment
JOIN                  dict
  ON                  asset.orchestration_environment.dictionaryID = dict.dictionaryID
JOIN                  meta.unique_attribute
  ON                  asset.orchestration_environment.dictionaryID = meta.unique_attribute.dictionaryID
JOIN                  asset.orchestration_environment_unique_attribute_values
  ON                  asset.orchestration_environment.dictionaryID = asset.orchestration_environment_unique_attribute_values.dictionaryID
 AND                  meta.unique_attribute.attributeID = asset.orchestration_environment_unique_attribute_values.attributeID
 AND                  asset.orchestration_environment.orchID = asset.orchestration_environment_unique_attribute_values.orchID
JOIN                  look
  ON                  meta.unique_attribute.attribute = look.key
JOIN                  uri
  ON                  asset.orchestration_environment.dictionaryID = uri.dictionaryID
WHERE                 asset.orchestration_environment.orchID = $2::uuid
  AND                 $3::timestamptz(3) <@ asset.orchestration_environment_unique_attribute_values.validity;`
)
