...
func (h *LibraryReadHandler) Intake() chan msg.Request {
	return h.Input
}
...
func (h *LibraryReadHandler) Run() {
	var err error

	for statement, prepared := range map[string]**sql.Stmt{
		stmt.LibraryList: &h.stmtList,
		stmt.LibraryShow: &h.stmtShow,
	} {
...
func (h *LibraryReadHandler) ShutdownNow() {
	close(h.Shutdown)
}
