...
func HandleRegister(hm *handler.Map, length int) {
	handleRegisterTechnicalProduct(hm, length)
	handleRegisterDeployment(hm, length)
	handleRegisterInstance(hm, length)
	handleRegisterShard(hm, length)
	handleRegisterEndpoint(hm, length)
	handleRegisterNetrange(hm, length)
}
