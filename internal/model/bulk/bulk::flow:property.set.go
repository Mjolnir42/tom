...
func init() {
	proto.AssertCommandIsDefined(proto.CmdFlowPropSet)

	registry = append(registry, function{
		cmd:    proto.CmdFlowPropSet,
		handle: flowPropSet,
	})
}

func flowPropSet(m *Model) httprouter.Handle {
	return m.x.Authenticated(m.FlowPropSet)
}

func exportFlowPropSet(result *proto.Result, r *msg.Result) {
	result.Flow = &[]proto.Flow{}
	*result.Flow = append(*result.Flow, r.Flow...)
}

// FlowPropSet function
func (m *Model) FlowPropSet(w http.ResponseWriter, r *http.Request,
	params httprouter.Params) {

	request := msg.New(
		r, params,
		proto.CmdFlowPropSet,
		msg.SectionFlow,
		proto.ActionPropSet,
	)
	...
	m.x.Send(&w, &result, exportFlowPropSet)
}

// propertySet ...
func (h *FlowWriteHandler) propertySet(q *msg.Request, mr *msg.Result) {
	var (
		txTime                                           time.Time
