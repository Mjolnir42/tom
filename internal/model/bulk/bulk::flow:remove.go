...
func init() {
	...
	registry = append(registry, function{
		cmd:    proto.CmdFlowRemove,
		handle: flowRemove,
	})
}

func flowRemove(m *Model) httprouter.Handle {
	return m.x.Authenticated(m.FlowRemove)
}

func exportFlowRemove(result *proto.Result, r *msg.Result) {
	result.Flow = &[]proto.Flow{}
	*result.Flow = append(*result.Flow, r.Flow...)
}

// FlowRemove function
func (m *Model) FlowRemove(w http.ResponseWriter, r *http.Request,
	...
	m.x.Send(&w, &result, exportFlowRemove)
}

// remove ...
func (h *FlowWriteHandler) remove(q *msg.Request, mr *msg.Result) {
	var (
		txTime                                           time.Time
	...
	// discover flowID at the start of the transaction
	if err = tx.Stmt(
		h.stmtTxShow,
	).QueryRow(
		q.Flow.Namespace,
