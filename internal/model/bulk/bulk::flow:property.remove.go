...
func init() {
	...
	registry = append(registry, function{
		cmd:    proto.CmdFlowPropRemove,
		handle: flowPropRemove,
	})
}

func flowPropRemove(m *Model) httprouter.Handle {
	return m.x.Authenticated(m.FlowPropRemove)
}

func exportFlowPropRemove(result *proto.Result, r *msg.Result) {
	result.Flow = &[]proto.Flow{}
	*result.Flow = append(*result.Flow, r.Flow...)
}

// FlowPropRemove function
func (m *Model) FlowPropRemove(w http.ResponseWriter, r *http.Request,
	...
	m.x.Send(&w, &result, exportFlowPropRemove)
}

// propertyRemove ...
func (h *FlowWriteHandler) propertyRemove(q *msg.Request, mr *msg.Result) {
	var (
		txTime                                           time.Time
	...
	for key := range q.Flow.Property {
		if _, ok = attrMap[q.Flow.Property[key].Attribute]; ok {
			reqMap[q.Flow.Property[key].Attribute] = attrMap[q.Flow.Property[key].Attribute]
		} else {
