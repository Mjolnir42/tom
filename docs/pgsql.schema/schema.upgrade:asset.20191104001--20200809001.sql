BEGIN;
  ...
  CREATE TABLE IF NOT EXISTS asset.socket_parent (
      ...
      CONSTRAINT __fk_asop_orchID   FOREIGN KEY ( parentOrchestrationID ) REFERENCES asset.orchestration_environment ( orchID ) ON DELETE RESTRICT,
      CONSTRAINT __validFrom_utc    CHECK       ( EXTRACT( TIMEZONE FROM lower( validity ) ) = '0' ),
      CONSTRAINT __validUntil_utc   CHECK       ( EXTRACT( TIMEZONE FROM upper( validity ) ) = '0' ),
      CONSTRAINT __asop_uq_parent   CHECK       (   ((parentRuntimeID IS NOT NULL) AND (parentOrchestrationID IS     NULL))
                                                 OR ((parentRuntimeID IS     NULL) AND (parentOrchestrationID IS NOT NULL))),
  ...
  INSERT INTO public.schema_versions ( schema, version, description )
              VALUES ( 'asset', 20200908001, 'add tables for socket in schema asset');
COMMIT;
