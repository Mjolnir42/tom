BEGIN;
  ...
  CREATE TABLE IF NOT EXISTS yp.service_linking (
      serviceID                     uuid        NOT NULL DEFAULT public.gen_random_uuid(),
      serviceDictionaryID           uuid        NOT NULL,
      endpointID                    uuid        NOT NULL,
      endpointDictionaryID          uuid        NOT NULL,
      CONSTRAINT __pk_ypsl          PRIMARY KEY ( serviceID ),
      CONSTRAINT __fk_ypsl_serID    FOREIGN KEY ( serviceID, serviceDictionaryID ) REFERENCES yp.service ( serviceID, dictionaryID ) ON DELETE RESTRICT,
      CONSTRAINT __fk_ypsl_enpID    FOREIGN KEY ( endpointID, endpointDictionaryID ) REFERENCES ix.endpoint ( endpointID, dictionaryID ) ON DELETE RESTRICT
  );
  ...
  INSERT INTO public.schema_versions ( schema, version, description )
              VALUES ( 'yp', 20200915002, 'add service tables');
COMMIT;
